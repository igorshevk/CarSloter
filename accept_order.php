...
$query=$conn->prepare("update tbl_order set order_status='1' where order_pid='$product_id' AND order_cid='$uid'");
...
$to =$useremail;
$subject = "Your bid has been placed # $order_no ";

$title = 'Your bid has been placed successfully!';
$subjectTitle = 'Your bid has been placed # '.$order_no;
$messageHeader = 'Hello '.$userfname.' '.$userlname.',';
$messageContent = 'Your bid has been placed successfully, you will receive an email notification with updates. Also please check My Account in website.';
...
// More headers
$headers .= 'From: CarSloter <bid-placed@carsloter.eu>' . "\r\n";
///$headers .= 'Cc: Carolgreen281@gmail.com' . "\r\n";
$a=mail($to,$subject,$message,$headers,'-fbid-placed@carsloter.eu');
$_SESSION['BID_MSG']='Congratulations, you placed the bid.';


if($row_product->category == 1)
  echo "<script>window.location.href='details.php?pid=$product_id' </script>";
else if($row_product->category == 2)
  echo "<script>window.location.href='truck-details.php?pid=$product_id' </script>";
else if($row_product->category == 3)
  echo "<script>window.location.href='farm-details.php?pid=$product_id' </script>";
}
?>