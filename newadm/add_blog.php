...
if(isset($_POST['save']))
{
	...
	$blog_img1=$_FILES['b_image']['name'];
	$img1_location=$_FILES['b_image']['tmp_name'];
	$date_time=date('d-m-Y h-i-s');
	$rand=rand();
	if($blog_img1!="")
	{
	$nblog_img1=$date_time.'-'.$rand.'-'.$blog_img1;
	} else 
	{
		$nblog_img1="";
	}
	move_uploaded_file($img1_location,"uploaded_file/blog/".$nblog_img1);
	
	$b_desc=$_POST['b_desc'];
	$b_status=$_POST['b_status'];
	
    $stmt=$conn->prepare("insert into tbl_blog(b_bcat_id,b_title,b_image ,b_desc,b_status)values(:b_bcat_id,:b_title,:nblog_img1,:b_desc,:b_status)");	

   $stmt->bindParam(":b_bcat_id",$b_bcat_id, PDO::PARAM_STR);
   $stmt->bindParam(":b_title",$b_title, PDO::PARAM_STR);
   $stmt->bindParam(":nblog_img1",$nblog_img1, PDO::PARAM_STR);
...
<head>
  <meta charset="utf-8">
    ...
    <link rel="icon" href="images/favicon.ico">
      ...
      <title><?=SITE_NAME;?> | Add Blog</title>
          ...
          <div class="row">
            <div class="col-12">
				  ...
				  <div class="col-sm-10">
					<select class="form-control" name="b_bcat_id">
					  ...
					  <?php $sql=$conn->prepare("select * from tbl_blog_category where bcat_status='1'");
							...
							{
					  ?>
					  <option value="<?php echo $row->bcat_id; ?>"><?php echo $row->bcat_name; ?></option>
						<?php } ?>
					</select>
				  </div>
				</div>
            	...
            	<div class="form-group row">
				  ...
				  <div class="col-sm-10">
					<textarea class="form-control" name="b_title" required="" placeholder="Blog Title"></textarea>
				  </div>
				</div>
				
				
				<div class="form-group row">
				  <label for="example-text-input" class="col-sm-2 col-form-label">Product Image1</label>
				  <div class="col-sm-10">
				...
				<div class="form-group row">
				  ...
				  <div class="col-sm-10">
					<textarea class="form-control" name="b_desc" required="" placeholder="Blog Description"></textarea>
				  </div>
				</div>
				...
				<div class="form-group row">
				  ...
				  <div class="col-sm-10">
					<input type="submit" class="btn btn-success" name="save" value="Save Blog">
				  </div>
				</div>
		     	</form>
