...
if(isset($_GET['supercatid']))
{
 ...
 $sql=$conn->prepare("delete from tbl_supercategory where supercat_id='$supercatid'");
...
<head>
  <meta charset="utf-8">
   ...
   <title><?=SITE_NAME;?> | Super Category List</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
	    <a href="add_super_category.php" class="btn btn-info">Add Super Category</a>
      </h1>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="myaccount.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="breadcrumb-item active">Super Category List</li>
      </ol>
    </section>
              ...
              <table id="example1" class="table table-bordered table-striped table-responsive">
                <thead>
					<tr style="background-color:#122F3B;">
						...
						<th style="width:30%;color:#fff;">Category Name</th>
				...
				<tbody>
					...
					<?php
					$stmt=$conn->prepare("select * from tbl_supercategory order by supercat_id DESC");
					$sno=1;
					$stmt->execute();
					...
					?>
					 <tr>
						<td><?=$sno;?></td>
						<td><?=$row->supercat_name;?></td>
						<?php
						 $status=$row->supercat_status;
                         if($status==1) {$status='Active'; } else {$status='Inactive';}
						?>
						<td><?php echo $status;  ?></td>
						<td>
						&nbsp;&nbsp;&nbsp;<a href="edit_super_category.php?supercatid=<?=$row->supercat_id;?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;
						&nbsp;&nbsp;&nbsp;<a href="super_category_list.php?supercatid=<?=$row->supercat_id;?>" onclick="return checkDelete()"><i class="fa fa-trash" aria-hidden="true"></i></a>
						</td>
					</tr>
					<?php $sno++; } ?>
				</tbody>
