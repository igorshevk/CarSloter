...
if(isset($_GET['brandid']))
{
 ...
 $data=$conn->prepare("select * from tbl_brand where brand_id='$brandid'");
 $res=$data->execute();
 $rows=$data->fetch(PDO::FETCH_OBJ);
 
 $sql=$conn->prepare("delete from tbl_brand where brand_id='$brandid'");
 $result=$sql->execute();
 if($result)
 {
	if(!empty($rows->brand_image))
		{
	unlink("uploaded_file/brand/".$rows->brand_image);
		} 
...
<head>
  <meta charset="utf-8">
   ...
   <title><?=SITE_NAME;?> | Brand List</title>
              ...
              <table id="example1" class="table table-bordered table-striped table-responsive">
                <thead>
					<tr style="background-color:#122F3B;">
						...
						<th style="width:25%;color:#fff;">Brand Title</th>
						<th style="width:25%;color:#fff;">Brand Image</th>
				...
				<tbody>
					...
					?>
					 <tr>
						...
						<td><img src="uploaded_file/brand/<?=$row->brand_image;?>" height="70" width="70"></td>
						<?php
						...
						<td>
						&nbsp;&nbsp;&nbsp;<a href="edit_brand.php?brandid=<?=$row->brand_id;?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;
						&nbsp;&nbsp;&nbsp;<a href="brand_list.php?brandid=<?=$row->brand_id;?>" onclick="return checkDelete()"><i class="fa fa-trash" aria-hidden="true"></i></a>
						</td>
					</tr>
					<?php $sno++; } ?>
				</tbody>
