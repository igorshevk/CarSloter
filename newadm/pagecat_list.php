...
{
 $pcatid=$_GET['pcatid'];
 $sql=$conn->prepare("delete from tbl_pagecat where pcat_id='$pcatid'");
 $result=$sql->execute();
 if($result)
 {
...
<head>
  <meta charset="utf-8">
   ...
   <title><?=SITE_NAME;?> | Page Category List</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
	    <a href="add_pagecat.php" class="btn btn-info">Add Page Category</a>
      </h1>
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="myaccount.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="breadcrumb-item active">Page Category List</li>
      </ol>
    </section>
            ...
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped table-responsive">
				...
				<tbody>
					...
					<?php
					$stmt=$conn->prepare("select * from tbl_pagecat order by pcat_id DESC");
					$sno=1;
					$stmt->execute();
					...
					?>
					 <tr>
						...
						<td><?=$row->pcat_name;?></td>
						<?php
						 $status=$row->pcat_status;
                         if($status==1) {$status='Active'; } else {$status='Inactive';}
						?>
						<td><?php echo $status;  ?></td>
						<td>
						&nbsp;&nbsp;&nbsp;<a href="edit_pagecat.php?pcatid=<?=$row->pcat_id;?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;
						&nbsp;&nbsp;&nbsp;<a href="pagecat_list.php?pcatid=<?=$row->pcat_id;?>" onclick="return checkDelete()"><i class="fa fa-trash" aria-hidden="true"></i></a>
						</td>
					</tr>
					<?php $sno++; } ?>
				</tbody>
