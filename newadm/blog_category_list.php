...
{
 $bcatid=$_GET['bcatid'];
 $sql=$conn->prepare("delete from tbl_blog_category where bcat_id='$bcatid'");
 $result=$sql->execute();
 if($result)
 {
...
<head>
  <meta charset="utf-8">
   ...
   <title><?=SITE_NAME;?> | Blog Category List</title>
            ...
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped table-responsive">
				...
				<tbody>
					...
					<?php
					$stmt=$conn->prepare("select * from tbl_blog_category order by bcat_id DESC");
					$sno=1;
					$stmt->execute();
					...
					?>
					 <tr>
						...
						<td><?=$row->bcat_name;?></td>
						<?php
						 $status=$row->bcat_status;
                         if($status==1) {$status='Active'; } else {$status='Inactive';}
						?>
						<td><?php echo $status;  ?></td>
						<td>
						&nbsp;&nbsp;&nbsp;<a href="edit_blog_category.php?bcatid=<?=$row->bcat_id;?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;
						&nbsp;&nbsp;&nbsp;<a href="blog_category_list.php?bcatid=<?=$row->bcat_id;?>" onclick="return checkDelete()"><i class="fa fa-trash" aria-hidden="true"></i></a>
						</td>
					</tr>
					<?php $sno++; } ?>
				</tbody>
